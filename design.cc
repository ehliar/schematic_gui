...
void schematic_design::trace_connref(Avoid::ConnRef *connref, bool startsearch, FILE *fp, int junctionlevel)
{
	...
	if(fp){
		...
		if(endpoints.first.shape()){
			...
			if(g->istext){
				fprintf(fp, "        $schematic_%stext(\"%s\", %d, %d, %d);\n",texttmp,
					g->gettext().c_str(), (int)g->getpos().x, (int)g->getpos().y, (int) endpoints.first.pinClassId());
			}else{
		...
		if(endpoints.second.shape()){
			...
			if(g->istext){
				fprintf(fp, "        $schematic_%stext(\"%s\", %d, %d, %d);\n",texttmp,
					g->gettext().c_str(), (int)g->getpos().x, (int)g->getpos().y, (int) endpoints.second.pinClassId());
			}else{
				fprintf(fp, "        $schematic_%s(%s.%s);\n", tmp,
					g->getinstancename().c_str(),
...
gate * schematic_design::create_new_gate(const char *filename, const char *instname)
{
	...
	printf("Creating new gate %s\n", filename);
        ...
        if(!strcmp(filename,".text")){
                ...
                i=-100000;  // (UGLY HACK: Unlikely to match a real instance!)
...
void schematic_design::save_verilog(std::string filename)
{
	...
	fprintf(fp,"    initial begin\n");
		...
		if(g->istext) {
                        Avoid::ConnRefList l = g->shapeRef->attachedConnectors();
                        if(l.begin() == l.end()){
                                fprintf(fp, "        $schematic_starttext(\"%s\", %d, %d, 0);\n", g->gettext().c_str(), (int)p.x, (int) p.y);
                        }
		}else{

                        fprintf(fp,"        $schematic_setgatepos(%s, %d,%d);\n", g->getinstancename().c_str(),
                                (int)p.x, (int) p.y);
			int rot = g->getrotation();
			if(rot){
				fprintf(fp, "        $schematic_setgaterot(%s, %d);\n", g->getinstancename().c_str(), rot);
			}
                }
...
void schematic_design::register_verilog_module(struct module *m)
{
	...
	Avoid::ConnRef *newconnref;
		...
		case SCHEMATICTEXT:
		{
			gate *textg = create_new_gate(".text");
